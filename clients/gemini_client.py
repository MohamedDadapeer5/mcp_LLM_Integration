...
class QABotMCPClient:
    """Gemini-powered Q&A Bot client that uses MCP tools for industry-specific knowledge."""
    ...
    def __init__(self, mcp_url: str = None):
        ...
        # Resolve a valid model that supports generateContent
        self._resolve_model(preferred)

    def _resolve_model(self, preferred_name: str):
        try:
        ...
        chosen = next((m for m in supported if simple(m.name) == preferred_name), None)

        # If not found, try common fallbacks in order
        if not chosen:
            fallbacks = [
                preferred_name,
                "gemini-1.5-flash",
            ...
            for fb in fallbacks:
                chosen = next((m for m in supported if simple(m.name) == fb), None)
                if chosen:
                    break

        # If still not found, pick the first supported
        if not chosen and supported:
            chosen = supported[0]

        if not chosen:
            available = ", ".join(simple(m.name) for m in models)
            raise RuntimeError(
                "No Gemini models supporting generateContent were found for your account. "
                f"Available models: {available}. Try setting GEMINI_MODEL to one of these."
            )

        self.model_name = simple(chosen.name)
    ...
    async def chat(self, user_message: str) -> str:
        """Chat with Gemini using MCP tools for context."""
        ...
        _ = detect_intent  # currently unused but kept for parity
        ...
        system_prompt = (
            f"You are a helpful Q&A bot for the {self.current_industry.title()} industry.\n"
        ...
        try:
            ...
            print("⚠ Rate limit or quota reached on Gemini. Please try again later.")
            escalation_id = await self._escalate_to_human("Gemini rate limit exceeded - user should retry")
            ...
            escalation_id = await self._escalate_to_human(f"Unexpected error: {str(e)}")
            return f"I encountered an error. Your case has been escalated (Ticket: {escalation_id})"
